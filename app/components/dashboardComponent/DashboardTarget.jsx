...
export default function DashboardTarget() {
  ...
  const { topics, lessons } = useAppContext()
    ...
    <div className="bg-greens-1000 rounded-lg pb-11">
      ...
      <div className="mt-6 px-6 sm:px-4 flex gap-2 items-center">
        <div className="w-20 aspect-square">
          <CircularProgressbar value={topics.reduce((total, obj) => total + obj.progress, 0) / (lessons.length * topics.length) * 100} strokeWidth={20} />
        </div>
        <div className="flex items-center gap-1">
          <p className="font-bold text-xl text-grays-900">{Math.floor(topics.reduce((total, obj) => total + obj.progress, 0) / (lessons.length * topics.length) * 100)}%</p>
